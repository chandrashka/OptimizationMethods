...
public class MNCConst
{
    public static void Calculate(double[] x0, double l)
    {
        Console.WriteLine();
        ...
        while(true)
        {
            ...
            Console.WriteLine($"f(x{i+1}) = {Test3.F(x)}");

            var newGrad = FindGradient(newX, out var newNorm);
            
            if (norm <= newNorm)
            {
                Console.WriteLine($"Функцiя зросла, тому необхiдно зменшити довжину кроку i \nповторити iтерацiю з точки x{i}. ");
                l /= 2;
            }
            else
            {
    ...
    private static double[] FindS(double[] grad, double norm)
    {
        var s0 = grad[0] / norm;
        var s1 = grad[1] / norm;

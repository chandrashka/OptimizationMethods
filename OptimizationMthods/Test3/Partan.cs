...
public class Partan
{
    public static void Calculate(double[][] x, double[] x0)
    {
        Console.WriteLine();
        Console.WriteLine("df/dx1 = 6x1 - 6 - x2,     df/dx2 = -x1 + 8x2");
        Console.WriteLine("f(x0) = " + Test3.F(x[0]));

        Console.WriteLine("Першi двi iтерацii аналогiчнi методу найшвидшого спуску. ");
        Console.WriteLine("Отриманi точки: ");
        Console.WriteLine($"x1 = ({x[0][0]}, {x[0][1]})^T");
        Console.WriteLine($"x2 = ({x[1][0]}, {x[1][1]})^T");
        double[][] results = [x[0], x[1], [0, 0]];

        Console.WriteLine($"\nIтерацiя 3");

        var s0 = FindS(x, x0);

        Console.WriteLine($"S = x2 - x0 =  ({x[1][0]},{x[1][1]})^T - ({x0[0]},{x0[1]})^T = ({s0[0]}, {s0[1]})^T");

        var grad = FindGradient(x[1], out var norm);
        var l = FindL(grad, s0);

        Console.WriteLine($"\nВиконується перехiд в точку x3");
        var newX = FindNewX(x[1], s0, l);
        Console.WriteLine($"x3 = ({newX[0]}, {newX[1]})^T");
        Console.WriteLine($"f(x3) = {Test3.F(newX)}");
    ...
    private static double[] FindS(double[][] x, double[] x0)
    {
        var s0 = x[1][0] - x0[0];
        var s1 = x[1][1] - x0[1];
